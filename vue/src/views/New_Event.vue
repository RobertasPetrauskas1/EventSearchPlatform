  ...
  <div>
    <div class="container">
        ...
        <div class="col-md-10">
          <div class="indicators d-flex justify-content-around py-4 text-light">
            <div
              ...
              :class="getIndicatorClass(1)"
            >1</div>
            <div
              class="rounded-circle bg-secondary px-2 shadow-sm mr-2"
              :class="getIndicatorClass(2)"
            >2</div>
            <div
              class="rounded-circle bg-secondary px-2 shadow-sm mr-2"
              :class="getIndicatorClass(3)"
            >3</div>
            <div
              class="rounded-circle bg-secondary px-2 shadow-sm mr-2"
              :class="getIndicatorClass(4)"
            >4</div>
          ...
          <div class="fix-height position-relative">
            ...
            <div class="steps" :class="getStepClass(1)">
              <div class="form-group">
                ...
                <v-select
                  :options="kategorijos"
                  :reduce="cat => cat.singular"
                  label="singular"
                  id="sel1"
                  v-model="eventCat"
                  :class="{invalid: !isValid('eventCat')}"
                />
              </div>
            </div>
          </div>
          <div class="steps" :class="getStepClass(2)">
            <div class="form-group">
              ...
              <input
                ...
                class="form-control"
                v-model="eventName"
                :class="{invalid: !isValid('eventName')}"
              />
              <div class="form-group">
                ...
                <textarea
                  ...
                  rows="3"
                  v-model="eventDes"
                  :class="{invalid: !isValid('eventDes')}"
                ></textarea>
              </div>
            </div>
          </div>
          ...
          <div class="steps" :class="getStepClass(4)">
            <div class="form-group">
              ...
              <input
                ...
                class="form-control"
                v-model="phoneNumb"
                :class="{invalid: !isValid('phoneNumb')}"
              />
              ...
              <input
                ...
                class="form-control"
                v-model="pageLink"
                :class="{invalid: !isValid('pageLink')}"
              />
            </div>
          </div>
          <div class="steps" :class="getStepClass(5)">
            <div class="form-group">
              ...
              <v-select
                ...
                v-model="eventPlace"
                :class="{invalid: !isValid('eventPlace')}"
              />
            </div>
            ...
            <div class="form-group row">
              <div class="col-10">
                <input
                  ...
                  v-model="eventDate"
                  :class="{invalid: !isValid('eventDate')}"
                />
              </div>
            </div>
            <div class="form-group row">
              <div class="col-10">
                <input
                  ...
                  v-model="eventTime"
                  :class="{invalid: !isValid('eventTime')}"
                />
              </div>
            </div>
          </div>
      ...
      <div class="col-md text-center">
        <button
          type="button"
          class="btnPrev btn btn-outline-success"
          @click="prevPage"
          v-show="pageNumber > 1"
        >Atgal</button>
        <button
          type="button"
          class="btnNext btn btn-outline-success"
          @click="nextPage"
          v-show="maxPageNumber > pageNumber"
        >Toliau</button>
        <button
          @click="createEvent"
          type="button"
          class="btnNext btn btn-success"
          v-show="maxPageNumber == pageNumber"
        >Išsaugoti</button>
      </div>
    </div>
...
let AllFields = {
  ...
  //nuotraukos truksta
  phoneNumb: {
    validate: v => v.match(phoneRegex) != null,
    page: 4
  },
  webSite: {
    validate: v => v.match(webRegex) != null,
    page: 4
  },
  pageLink: {
    validate: v => v.match(webRegex) != null,
...
export default {
  ...
  methods: {
    ...
    validateField(name) {
      ...
      // debugger;
      if (!AllFields[name].validate(value)) {
        return false;
      }
      if (value) {
        return true;
    ...
    readCategoryTypes() {
      axios
        ...
        .then(res => {
          this.kategorijos = res.data;
        })
        .catch(err => console.error(err));
    },
    readPlaceTypes() {
      axios
        .get(`${c.serverURL}/place/allNames`)
        .then(res => {
